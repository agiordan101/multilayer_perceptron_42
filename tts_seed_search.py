...
def get_model(tts_seed=None):
	...
	# model = annpy.models.sequential(input_shape=name="First model")
...
def get_model_train(tts_seed=None):

	model = get_model(tts_seed)
...
for i in range(n_seed_search):
	...
	randn = random.choice(seed[1]) % 1000000
...
print(f"\tBEST TTS_SEED ({n_seed_search}) -- loss: {loss_}\n")

if best_loss < best_loss_file:

	tts_seed = best_model.get_seeds()['tts_seed']
